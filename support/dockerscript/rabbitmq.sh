...
if [ "$1" = "start" ]; then
    ...
    docker run -d --name $CONTAINER_NAME $NETWORK_OPTION -p 5672:5672 -p 15672:15672 -p 5671:5671 -p 15671:15671 -p 4369:4369 -e RABBITMQ_DEFAULT_USER=guest -e RABBITMQ_DEFAULT_PASS=guest $IMAGE_NAME
elif [ "$1" = "stop" ]; then
    # 컨테이너가 이미 존재하는지 확인
    if [ "$(docker ps -aq -f name=$CONTAINER_NAME)" ]; then
